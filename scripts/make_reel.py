...
# ✅ Render text using pango-view (perfect Devanagari shaping)
def render_with_pango_view(text: str, out_png: Path, font_desc: str, width_px: int):
    safe = xml_escape(text)

    # pango-view requires markup or plain
...
def main():
    ...
    def make_frame(t):
        ...
        x1 = (W - zw) // 2
        y1 = (H - zh) // 2
        bg = bg.crop((x1, y1, x1 + zw, y1 + zh)).resize((W, H), Image.Resampling.LANCZOS)
        ...
        if a_hi > 0:
            ...
            paste_centered(bg, Image.open(hi_png), W // 2, 1240, alpha=a_hi)
    ...
    print("✅ Reel created (Hindi/Sanskrit fixed via pango-view):", OUT_VIDEO)
